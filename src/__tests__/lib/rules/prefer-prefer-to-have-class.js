...
ruleTester.run("prefer-to-have-class", rule, {
  valid: [
    ...
    `expect(el).toHaveProperty("className", "foo")`
  ],
  invalid: [
    {
      code: `expect(screen.getByRole("button").className).toBe("foo")`,
      errors,
      output: `expect(screen.getByRole("button")).toHaveClass("foo")`
    },
    {
      code: `expect(screen.getByRole("button").className).not.toBe("foo")`,
      errors,
      output: `expect(screen.getByRole("button")).not.toHaveClass("foo")`
    },
    {
      code: `expect(el).toHaveProperty("class", "foo")`,
      errors,
    ...
    {
      code: `expect(el).not.toHaveProperty("class", "foo")`,
      errors,
