...
export const create = (context) => ({
    //expect(element).not.toHaveAttribute('value', 'foo') / Property
    [`xCallExpression[callee.property.name=/toHave(Attribute|Property)/][callee.object.object.callee.name=expect][callee.object.property.name=not]`](node) {
        context.report({
            messageId,
            node,
            fix(fixer) {
    ...
    [`CallExpression[callee.property.name=/to(Be|(Strict)?Equal)/][callee.object.arguments.0.property.name=value][arguments.0][callee.object.callee.name=expect]`](node) {
        ...
        context.report({
            ...
            fix(fixer) {
                return [
                    fixer.removeRange([node.callee.object.arguments[0].object.range[1], node.callee.object.arguments[0].property.range[1]]),
    ...
    [`CallExpression[callee.property.name=/toHave(Attribute|Property)/][arguments.0.value=value][arguments.1][callee.object.callee.name=expect], CallExpression[callee.property.name=/toHave(Attribute|Property)/][arguments.0.value=value][arguments.1][callee.object.object.callee.name=expect][callee.object.property.name=not]`](node) {
        ...
        context.report({
            ...
            fix(fixer) {
                return [
                    fixer.replaceText(node.callee.property, "toHaveValue"),
                    fixer.removeRange([node.arguments[0].range[0], node.arguments[1].range[0]])
                ];
            }
        });
    }
